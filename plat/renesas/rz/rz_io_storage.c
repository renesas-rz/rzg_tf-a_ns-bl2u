...
static const io_spi_flash_spec_t flash_main_block_spec = {
	.device_address = FLASH_BASE,
	.region_address = FLASH_BASE,
	.block_size = SPI_FLASH_BLOCK_SIZE,
	.block_count = FLASH_SIZE / SPI_FLASH_BLOCK_SIZE
};

int rz_io_setup(void)
{
	int io_result;

	io_result = register_io_dev_spi_flash(&flash_dev_con);
